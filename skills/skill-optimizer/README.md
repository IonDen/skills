...
| `MOVED_TO_REFERENCE` | A rule or an anchored sentence now lives only in a new reference file, or a literal that ran across two sentences moved there with both and no longer reads as written (exit 3, not a rejection) |
...
| `EMPHASIS_LOST` | A rule, a rule heading or an anchored sentence lost bold or italic it had, or its bold became italic (exit 3, not a rejection). Emphasis kept or added passes |

The first thirteen codes reject the candidate outright. `MOVED_TO_REFERENCE`, `SECTION_CHANGED` and `EMPHASIS_LOST` don't reject. The workflow puts each such rule back as it was and lists the change under "Optional further cuts (not applied)", unless your request already approved moving that item. A rule sentence, a rule heading or an anchored sentence can also be deleted outright, but only after you approve that exact text: it goes into `approved.txt`, one sentence per line, copied exactly, and the gate is re-run with `--approved approved.txt`. The text has to be gone whole. If a candidate sentence keeps some of its words in order ("Deploy on Fridays." left from "Deploy on Fridays only when the lead signs off."), the gate reads it as trimmed, not deleted, names the sentence left over, and still rejects it. A literal that appeared only in sentences you approved and that were deleted whole goes with them, including one that runs across two of them (`under 20` at the end of one paragraph, `GiB` at the start of the next). A literal whose parts no approvable sentence holds, such as a bound whose `>` a line wrap pushed to the start of the next line, or one that runs into a heading, cannot be approved away: the gate keeps rejecting it and the text stays. Without that file, deleting a rule sentence is `RULE_LOST` and the candidate is rejected.
...
**1.1.0**: A new code, `EMPHASIS_LOST`, asks before a rule, a rule heading or an anchored sentence loses its bold or italic; before, the gate compared text with emphasis removed and let a stripped rule lead-in through. A literal that runs across two sentences can now be deleted with both of them when you approve both, and when both move together into a reference where it no longer reads as written, the gate asks instead of rejecting. The lines of one blockquote are read as one paragraph, so `> under 20` / `> GiB` is protected as `under 20 GiB`, and a bound whose `>` was wrapped to the start of a line keeps its comparator. An anchor's three-word minimum counts words the way the gate reads them, so "Subject–verb proximity" is three words. The docs now say how to anchor around a hyphen at a line end and in a table, and that a "Common mistakes" table is not to be cut as duplicate text.

**1.0.2**: The freeze also reads each table cell, list item and paragraph on its own, and keeps a match that spans two of them only when the file holds it as written. Before, a bound at the end of one table row ran into the first word of the next (`≤ 0.20` plus `Pixel`), and the gate rejected the unchanged skill because that joined text never appears in the file.
