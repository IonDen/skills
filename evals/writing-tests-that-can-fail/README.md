...
After the runs, a review found that the held-out suite for fixture F held only whole and exact-half amounts, so a fix that always rounds up would have passed it. Two cases below the half (179.1 and 1.3 cents) were added, and the red-test scoring now allows new test files and lists them in `tests_added` instead of treating them as a changed test. Every recorded F verdict was recomputed; all seven are still `fixed`, and the only change is the new field.
