...
| Skill | What it does |
|---|---|
| [subagent-optimizer](skills/subagent-optimizer/) | For subagent definitions, the `.claude/agents/*.md` files. Audits them and cuts what each agent costs to launch: a tight `tools` allowlist, less prompt bloat, a model that fits the job. One real agent went from 20,122 to 13,962 launch tokens with the same result. |
